		...
		mainPanel(
			tabsetPanel(
				tabPanel("Project Scope",
						 ...
						 h4("Objective"),
						 p("The objective of this project would be to apply a predictive model on the wind turbine’s aggregated data and try to predict the generator’s winding temperature during its effective lifetime in a specific location and component configuration. This will allow us to have a time series visualization of the variations in winding temperature as seen in the plot below. Our aim is to shift the threshold of the wind turbines either above or below the 155 degrees threshold, based on the total area under the graph of winding temperature variations.  This will allow us to build a model that will maximize energy production of the wind turbine (more specifically generator’s itself) through its effective lifetime and thus, maximize the economical return of the wind turbine."),
						 img(src = "temp_tresholds.bmp", width = "100%")
						   ),
				tabPanel("Table", 
						 ...
						 br(),
						 p("Note: Data set is taken of the EWP-3120 PLC SN103.")
				),
				tabPanel("Histogram", plotOutput("simple_plot")),
				tabPanel("Linear Regression", plotOutput("Var_vs_Wind")),
				tabPanel("PCA Analysis", 
						 p("PCs"),
						 img(src = "pcs.png", width = "100%"),
						 br(),
						 p("PCs Statistics"),
						 img(src = "pcs_stats.png", width = "100%"),
