...
gapminder %>% 
  group_by(country) %>% 
  mutate(popChange = pop[year==1972]-pop[year==1962]) %>% 
  mutate(popBillions = pop/10^9) %>% 
  mutate(gdpLag = lag(gdpPercap, k=5))

gapminder %>% 
  filter(year %in% c(1062, 1972)) %>% 
  arrange(year) %>% 
  group_by(country) %>% 
...
- Put the y-axis on a log-scale. - DONE
...
gapminder %>% 
  ...
  facet_wrap(~ continent) +
  geom_line(aes(group=country), colour=log(max_gdpPercap)) +
  scale_y_log10(breakes = c(0.1, 1, 10, 100, 1000),
