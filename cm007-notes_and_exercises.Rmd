...
vc1 + geom_smooth(se=FALSE) + aes(colour=continent)
vc1 + geom_smooth(method="lm")
```
...
```{r}
vc1 <- ggplot(gapminder, aes(year, lifeExp, colour=continent))
vc1 + geom_smooth(method="lm", se=FALSE)
vc1 + geom_point() 
```
...
As for other arguments of `facet_wrap` that I find to be most useful, check the documentation for `scales` and `ncol` (# of columns in the grid) -- and if you're brave, `labeller`. 
