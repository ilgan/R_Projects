...
```{r}
p2 + geom_point(alpha=0.25, colour = "red", aes(size=year))
...
```{r}
p2 + aes(colour = continent) + geom_point()
```
...
```{r}
p22 <- ggplot(gapminder, aes(x=log10(gdpPercap), y=lifeExp))
p22 + geom_point() + scale_x_log10()
```
...
```{r}
p23 <- ggplot(gapminder, aes(x=year, y=lifeExp)
p23 + geom_point() + scale_x_log10() + aes(colour = continent)
```
