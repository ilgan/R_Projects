...
https://github.com/JoeyBernhardt/stat545-purrr/edit/master/purrr-lesson.Rmd
...
map_chr(.x = fruit, function(x) str_split(x, pattern = " "))
...
```{r}
list <- c("login", "name", "id", "location")
gh_users %>% 
  map(list)

x <- map(gh_users, `[`, c("login", "name", "id", "location"))
map(gh_users, magrittr::extract, list)

gp_mean <- map(gapminder$pop, mean)
```
